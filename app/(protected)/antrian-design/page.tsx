...
export default function AntrianDesignPage() {
  ...
  const [activeTab, setActiveTab] = useState<DesignStage>('WAITING');
  ...
  const stageCounts = useMemo(() => {
    const c: Record<DesignStage, number> = {
      WAITING: 0, AWAL: 0, REVISI_1: 0, REVISI_2: 0, REVISI_3: 0, SELESAI: 0,
    };
  ...
  // WAITING → AWAL. Dipanggil dari tombol 'Mulai Proses' di kartu
  // yang stage-nya WAITING. Reset design_stage_started_at supaya SLA
  // Design Awal (+2 hari) mulai dihitung dari saat designer pick-up,
  // bukan dari saat Finance approve.
          ...
          <div className="px-4 py-12 text-center">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto">
              {activeTab === 'WAITING'
                ...
                : activeTab === 'AWAL'
                  ? 'Klik Mulai Proses di Waiting List untuk memindah order ke Design Awal.'
...
function OrderCard({
  ...
  const isWaiting = currentStage === 'WAITING';
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isWaiting ? (
              <button
                onClick={onMulai}
                disabled={busy}
                title="Ambil order — pindah ke Design Awal, SLA +2 hari kerja mulai dihitung."
