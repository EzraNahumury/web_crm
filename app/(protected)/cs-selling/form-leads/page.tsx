...
export default function FormLeadsPage() {
  ...
  const totalLeadsAll = rows.reduce((s, r) => s + (Number(r.jumlah_leads) || 0), 0);
  const totalClosinganAll = rows.reduce((s, r) => s + (Number(r.jumlah_closingan) || 0), 0);
    ...
    <div className="space-y-5">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-gradient-to-br from-cyan-500/[0.14] via-blue-500/[0.06] to-transparent p-5 sm:p-6">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Form Leads CS</h1>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[400px_minmax(0,1fr)] gap-5 items-start">
        ...
        <div className="rounded-2xl bg-[#111827] border border-white/[0.06] overflow-hidden">
          <div className="px-4 py-3 border-b border-white/[0.06]">
            <p className="text-sm font-semibold text-white">History Catatan</p>
            <p className="text-[11px] text-slate-500">{rows.length} catatan, urut terbaru dulu</p>
          </div>
                ...
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-10 text-center text-sm text-slate-500">
                      Belum ada catatan. Isi form di sebelah kiri.
                    </td>
                  </tr>
                ) : (
                  rows.map(r => {
                    ...
                    return (
                      <tr key={r.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                        <td className="px-4 py-2.5 text-slate-300 font-medium">{fmtDayShort(r.tanggal)}</td>
