...
export default function ProduksiPage() {
  ...
  const [visualisasiOpen, setVisualisasiOpen] = useState(false);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-gradient-to-br from-emerald-500/[0.12] via-teal-500/[0.05] to-transparent p-5 sm:p-6">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={() => setVisualisasiOpen(true)}
              title="Lihat visualisasi beban antrian vs kapasitas per stage"
              className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold px-3.5 py-2.5 rounded-xl transition-colors shadow-lg shadow-blue-500/20"
            >
      ...
      {visualisasiOpen && (
        <VisualisasiModal
          onClose={() => setVisualisasiOpen(false)}
...
/* ─────────────────────────────────────────────────────────────────────
   VisualisasiModal — 2 grafik yang menunjukkan beban antrian saat ini
   dibanding kapasitas harian per stage. Chart pakai Recharts (LineChart)
   sesuai contoh line chart di briefing.
   Chart 1: stage per WO (Proofing 15/hari).
   Chart 2: stage per pcs (Printing Layout dst).
   ───────────────────────────────────────────────────────────────────── */

interface VisualisasiModalProps {
  onClose: () => void;
  progress: Row[];
  wos: Row[];
  stages: Row[];
...
function VisualisasiModal({ onClose, progress, wos, stages }: VisualisasiModalProps) {
  // Peta stage id → nama dari data DB (bukan hard-coded label).
  const stageById = useMemo(() => {
    const m: Record<number, string> = {};
    for (const s of stages) m[Number(s.id)] = String(s.nama);
    return m;
  }, [stages]);

  // Untuk tiap stage yang punya kapasitas, hitung beban antrian saat ini:
  // - Unit WO: count wo_progress TERSEDIA/SEDANG.
  // - Unit pcs: sum jumlah dari WO yang wo_progress-nya TERSEDIA/SEDANG.
  const rows = useMemo(() => {
    // work_orders lookup untuk sum pcs
    const woById: Record<number, Row> = {};
    for (const w of wos) woById[Number(w.id)] = w;
    ...
    for (const [stage, cap] of Object.entries(STAGE_CAPACITY)) {
      if (!cap) continue;
      const stageRow = stages.find(s => String(s.nama) === stage);
      if (!stageRow) continue;
      const stageId = Number(stageRow.id);
      const progressRows = progress.filter(p =>
        Number(p.stage_id) === stageId && (p.status === 'TERSEDIA' || p.status === 'SEDANG')
      );
      let current = 0;
      if (cap.unit === 'wo') {
        current = progressRows.length;
      } else {
        current = progressRows.reduce((sum, p) => {
          const wo = woById[Number(p.work_order_id)];
          return sum + (Number(wo?.jumlah) || 0);
        }, 0);
      }
      list.push({
        stage,
        shortLabel: stage.replace('QC Final dan Packing', 'QC Final')
          .replace('QC Panel Process', 'QC Panel')
          .replace('Printing Layout', 'Print Layout')
          .replace('Printing Process', 'Printing')
          .replace('Sublim Press', 'Sublim')
          .replace('Fabric Cutting', 'Fabric'),
        unit: cap.unit,
        limit: cap.limit,
        current,
      });
    ...
    return list;
  }, [progress, wos, stages, stageById]);

  const woRows = rows.filter(r => r.unit === 'wo');
  const pcsRows = rows.filter(r => r.unit === 'pcs');
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 overflow-y-auto pointer-events-none">
        <div
          className="bg-[#0f1626] border border-white/[0.08] rounded-2xl shadow-2xl w-full max-w-5xl my-8 flex flex-col max-h-[92vh] pointer-events-auto"
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] bg-gradient-to-r from-blue-500/[0.10] to-transparent">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">Visualisasi Beban Antrian</h3>
                <p className="text-xs text-slate-500 mt-0.5">Beban antrian saat ini vs kapasitas harian per stage.</p>
              </div>
            </div>
          ...
          {/* Body: 2 chart section */}
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            <ChartSection
              title="Stage per WO"
              subtitle="Stage yang kapasitasnya dihitung per Work Order (Proofing)."
              rows={woRows}
              unitLabel="WO"
              accent="cyan"
            />
            <ChartSection
              title="Stage per Pcs"
              subtitle="Stage produksi fisik yang kapasitasnya dihitung per pcs."
              rows={pcsRows}
              unitLabel="pcs"
              accent="emerald"
            />
...
// Line chart section — 2 line (Antrian sekarang vs Kapasitas harian).
// Pakai Recharts LineChart sesuai gaya contoh 'Data Penjualan' di briefing.
function ChartSection({
  title, subtitle, rows, unitLabel, accent,
}: {
  title: string;
  subtitle: string;
  rows: { stage: string; shortLabel: string; unit: string; limit: number; current: number }[];
  unitLabel: string;
  accent: 'cyan' | 'emerald';
}) {
  const currentColor = accent === 'cyan' ? '#22d3ee' : '#34d399';
  const currentColorDim = accent === 'cyan' ? 'rgba(34, 211, 238, 0.15)' : 'rgba(52, 211, 153, 0.15)';
  const overLimit = rows.some(r => r.current > r.limit);

  if (rows.length === 0) {
    return (
      <div className="rounded-xl bg-[#0d1117] border border-white/10 p-6 text-center">
        <p className="text-sm text-slate-500">Tidak ada stage dengan unit {unitLabel} yang aktif.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl bg-[#0d1117] border border-white/10 overflow-hidden">
      <div className="px-5 py-3 border-b border-white/[0.06] flex items-start justify-between gap-3 flex-wrap">
        <div>
        ...
        <div className="flex items-center gap-3 text-[11px]">
          <span className="inline-flex items-center gap-1.5">
            <span className="w-3 h-0.5" style={{ backgroundColor: currentColor }} />
            <span className="text-slate-300">Antrian ({unitLabel})</span>
          </span>
          <span className="inline-flex items-center gap-1.5">
            <span className="w-3 h-0.5 border-t border-dashed border-red-400" />
            <span className="text-slate-300">Kapasitas harian</span>
          </span>
        </div>
      </div>
      <div className="p-4">
        <RechartLine rows={rows} currentColor={currentColor} currentColorDim={currentColorDim} unitLabel={unitLabel} />
      </div>
      {overLimit && (
        <div className="px-5 py-3 border-t border-red-500/25 bg-red-500/[0.06] text-[11px] text-red-200 flex items-start gap-2">
          <svg className="w-3.5 h-3.5 shrink-0 mt-0.5 text-red-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
          </svg>
          <span>
            Ada stage yang beban antriannya <strong>melebihi</strong> kapasitas harian — cek titik yang di atas garis merah putus-putus.
          </span>
        </div>
      )}
    </div>
...
function RechartLine({
  rows, currentColor, currentColorDim, unitLabel,
}: {
  rows: { stage: string; shortLabel: string; unit: string; limit: number; current: number }[];
  currentColor: string;
  currentColorDim: string;
  unitLabel: string;
}) {
  // Import Recharts lazily via dynamic import supaya bundle main
  // Produksi tidak membengkak kalau modal tidak dibuka.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const max = Math.max(...data.map(d => Math.max(d.antrian, d.kapasitas))) * 1.15;
      setChart(
        <R.ResponsiveContainer width="100%" height={280}>
          <R.LineChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 40 }}>
            ...
            <R.YAxis
              ...
              width={45}
              label={{ value: unitLabel, position: 'insideLeft', angle: -90, fill: '#64748b', fontSize: 11, dx: 10 }}
            />
            <R.Tooltip
              contentStyle={{
            ...
            <R.Line
              type="monotone"
              dataKey="antrian"
              name={`Antrian (${unitLabel})`}
              stroke={currentColor}
              strokeWidth={2.5}
              dot={{ r: 4, fill: currentColor, stroke: currentColorDim, strokeWidth: 3 }}
              activeDot={{ r: 6 }}
            />
            <R.Line
              type="monotone"
              dataKey="kapasitas"
              name="Kapasitas harian"
              stroke="#f87171"
              strokeDasharray="6 4"
              strokeWidth={2}
              dot={{ r: 3, fill: '#f87171' }}
