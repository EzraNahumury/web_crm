...
export default function StokPage() {
  ...
  const [forecastBahan, setForecastBahan] = useState<Row[]>([]);
  async function fetchData() {
    try {
      const [s, a, b, tb, g, w, fb] = await Promise.all([
        dbGet('stok').catch(() => []),
  ...
  // Forecasting deficit: agregat kebutuhan bahan dari semua wo_forecast_bahan,
  // bandingkan dengan qty stok. Kalau kebutuhan > stok, munculkan warning
  // banner + list per bahan defisit.
  const forecastDeficits = useMemo(() => {
    const needed: Record<string, number> = {};
    for (const f of forecastBahan) {
      const nama = String(f.bahan || '').trim().toUpperCase();
      if (!nama) continue;
      needed[nama] = (needed[nama] || 0) + (Number(f.kuantitas) || 0);
    }
    const out: { nama: string; needed: number; stok: number; satuan: string; defisit: number }[] = [];
    for (const nama in needed) {
      if (needed[nama] <= 0) continue;
      const row = stokDisplay.find(m => m.nama.toUpperCase().trim() === nama);
      const stok = row?.qty ?? 0;
      if (needed[nama] > stok) {
        out.push({
          nama, needed: needed[nama], stok,
          satuan: row?.satuan || '-',
          defisit: needed[nama] - stok,
        });
      }
    }
    out.sort((a, b) => b.defisit - a.defisit);
    return out;
  }, [forecastBahan, stokDisplay]);
    ...
    <div className="space-y-5">
      ...
      {tab === 'overview' && (
        <div className="space-y-5">
          ...
          {/* Forecasting deficit warning — muncul kalau ada bahan yang
              kebutuhan forecast > stok. Bukan blocking, cuma notes. */}
          {forecastDeficits.length > 0 && (
            <div className="rounded-2xl bg-gradient-to-br from-amber-500/[0.12] to-transparent border border-amber-500/25 p-5">
              <div className="flex items-start gap-3 mb-3">
                <div className="w-9 h-9 rounded-lg bg-amber-500/20 border border-amber-500/30 grid place-items-center shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-base font-semibold text-amber-100">
                    Kekurangan Stok untuk Forecasting ({forecastDeficits.length} bahan)
                  </h2>
                  <p className="text-xs text-amber-200/70 mt-0.5">
                    Kebutuhan dari sub-menu <strong className="text-white">Forecasting Bahan</strong> melebihi stok. Segera restock supaya siap produksi.
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {forecastDeficits.slice(0, 12).map(d => (
                  <div key={d.nama} className="rounded-lg border border-amber-500/20 bg-amber-500/[0.05] px-3 py-2">
                    <p className="text-xs font-semibold text-white truncate" title={d.nama}>{d.nama}</p>
                    <p className="text-[11px] text-amber-200/80 mt-0.5">
                      Perlu <strong>{d.needed}</strong> · Stok <strong>{d.stok}</strong> {d.satuan}
                      <span className="text-rose-300 font-semibold ml-2">−{d.defisit}</span>
                    </p>
                  </div>
                ))}
              </div>
              {forecastDeficits.length > 12 && (
                <p className="text-[11px] text-amber-200/60 mt-2 text-center">
                  … dan {forecastDeficits.length - 12} bahan lainnya
                </p>
              )}
            </div>
          )}
