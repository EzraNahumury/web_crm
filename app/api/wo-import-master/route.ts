...
//   2. Convert the single-sheet .xlsx to PDF via LibreOffice (headless).
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const sheetName of sheetNames) {
      ...
      // Only WO 1 sheets go through LibreOffice → PDF → PNG rasterization
      ...
      const pages: string[] = [];
      if (target === 'wo1' && convertAsync) {
        try {
          const pdfBuf = await convertAsync(subBuffer, '.pdf');
          ...
          console.warn('Sheet conversion failed:', sheetName, e);
        }
      }
    ...
    return NextResponse.json({
      ...
      master: masterUrl,
      results,
      libreofficeAvailable: !!convertAsync,
    });
  } catch (e) {
